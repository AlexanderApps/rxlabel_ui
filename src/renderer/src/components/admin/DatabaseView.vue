<template>
  ...
  <!-- Show Backup Settings View -->
  <BackupSettingsView
    v-else-if="showBackupSettings"
    @back="showBackupSettings = false"
    @backup-created="handleBackupCreated"
    @settings-saved="handleSettingsSaved"
  />

  <!-- Show Optimize Database View -->
  <OptimizeDatabaseView
    v-else-if="showOptimizeView"
    @back="showOptimizeView = false"
    @optimization-complete="handleOptimizationComplete"
  />

  <!-- Show Manage Keys View -->
  <ManageKeysView v-else-if="showManageKeys" @back="showManageKeys = false" />
...
const ImportDataView = defineAsyncComponent(() => import('./ImportDataView.vue'))
const ExportDataView = defineAsyncComponent(() => import('./ExportDataView.vue'))
const BackupSettingsView = defineAsyncComponent(() => import('./BackupSettingsView.vue'))
const OptimizeDatabaseView = defineAsyncComponent(() => import('./OptimizeDatabaseView.vue'))
const ManageKeysView = defineAsyncComponent(() => import('./ManageKeysView.vue'))
const BackupKeyModal = defineAsyncComponent(() => import('./BackupKeyModal.vue'))
...
const showOptimizeView = ref(false)
const showManageKeys = ref(false)
...
const dbActions = [
  {
    ...
    action: () => (showBackupSettings.value = true)
  },
  {
    label: 'Import Data',
  ...
  },
  {
    ...
    action: () => (showOptimizeView.value = true)
  },
  {
    label: 'Rekey Database',
    iconPath:
    ...
    color: 'bg-gradient-to-r from-amber-500 to-orange-600 text-white',
    action: () => (showManageKeys.value = true)
  },
  {
    label: 'Create Backup',
    iconPath:
...
const handleBackupCreated = async (result) => {
  if (result.success) {
    alerts.success('Manual backup created successfully')
    await getBackups()
    await getLastBackupDate()
  } else {
    alerts.error(`Backup failed: ${result.error}`)
  }
}

const handleSettingsSaved = (settings) => {
  alerts.success('Backup settings saved successfully')
  console.log('Updated settings:', settings)
}

const handleOptimizationComplete = async (result) => {
  alerts.success(`Database optimized! ${result.spaceSaved} recovered in ${result.duration}s`)
  await fetchGeneralStats()
}
